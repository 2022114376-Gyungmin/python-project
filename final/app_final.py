...
if st.button("분석 시작", type = "primary", use_container_width = True):
    ...
    # NDVI 값 의미 설명 (펼침)
    with st.expander("💡 NDVI 값 해석 가이드", expanded = False):
        st.markdown("""
        **NDVI (정규화 식생 지수)**: -1 ~ 1 범위의 식생 건강도 지표

        | NDVI 값 | 의미 |
        |---------|------|
        | 0.6 ~ 0.9 | 🌲 건강한 식생 (울창한 숲 등) |
        | 0.2 ~ 0.5 | 🌱 잔디, 농작물 등 |
        | 0 근처 | 🏙️ 맨땅, 바위, 도시(건물) |
        | 음수 | 💧 물, 눈, 구름 |
    ...
    if result['percent_change'] < 0:
        ...
        함지산 산불 피해 지역의 식생 지수가 **{abs(result['percent_change']):.1f}%** 감소했습니다.
        이는 산불로 인한 식생피해를 명확하게 보여줍니다.
        """)
    else:
        st.success("식생 변화가 감지되지 않았습니다.")
